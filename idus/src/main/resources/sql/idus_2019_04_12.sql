...
create or replace view postComment_view as 
	select postComment.comNo, postComment.postNo, member.memberNo, member.nickName, member.thumbnail, postComment.content, postComment.createdDate, postComment.modifiedDate 
...
create or replace view itemListRecent as 
	...
	from (piece inner join pieceImage on piece.pieceNo=pieceImage.pieceNo) inner join member on member.memberNo = piece.artistNo
